  ...
  describe ".where" do
    context "not using cache" do
      it "returns a list of pull requests" do
        some_date = DateTime.parse("1985/08/06 11:00:00").in_time_zone # May or may not be my birtday
        ...
        allow(GithubPullRequest).to receive(:fetch)
          ...
          .and_return([
                        GithubPullRequest.new(link: "http://example.com", created_at: Time.zone.now, number_of_comments: 5)
                      ])

        expect(described_class.where(status: :open, repositories: available_repositories, cache: false)).to eq(expected_collection)
    ...
    context "using cache" do
      ...
      it "returns a list of pull requests" do
        ...
        expect(described_class.where(status: :open, repositories: available_repositories, cache: true)).to eq(expected_collection)
      end
    end
  end
end
